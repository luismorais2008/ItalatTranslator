...
function audio(){
    const audio = new Audio('https://www.televisiontunes.com/uploads/audio/Universal%20Studios.mp3');
    audio.play();
}
...
async function get_translation(text) {
    ...
    paragraph.innerText = 'A traduzir...'
    const ar = splitMulti(text, [",", "...", "!", "?", "\"", "'", "(", ")", ";", ".", ":"])
    const response = await fetch('http://149.90.143.191/translate?text=' + String(ar[0]));
    txt = String(await response.text())
    paragraph.innerText = txt
    ar = ar.slice(1, ar.length-1)
    for(let a in ar){
        const response2 = await fetch('http://149.90.143.191/translate?text=' + String(a));
        txt = String(await response2.text())
        paragraph.innerText += txt
    }
}
...
def index(request):
    if (request.method == "GET"):
        try:
            ...
            pass
    return HttpResponse("Ocorreu um erro enquanto o programa tentava traduzir a sua extensa mensagem.")
...
def texto_portuguese_to_italat(text):  # dividir o texto em orações
    ...
    try:
        ...
        for i in range(len(c)-1):
            translation += word_portuguese_to_italat(
                s[c[i]]) + " " + oracao_portuguese_to_italat(' '.join(s[c[i]+1:c[i+1]])) + " "
        translation += word_portuguese_to_italat(word_data(
            s[c[len(c)-1]])) + " " + oracao_portuguese_to_italat(' '.join(s[c[len(c)-1]+1:len(s)]))
    except:
        translation = oracao_portuguese_to_italat(' '.join(s[0:len(s)-1]))
    return translation


# organizar a sintaxe da oração e dividir a oração em palavras
def oracao_portuguese_to_italat(text):
    text = text.replace("-", "")
    palavras = []
    verb = False
    for p in text.split():
...
def word_data(word):  # recolher informação sobre uma palavra
    ...
    if "verbo" in priberam:
        ...
        if "Conj" in palavra["infinitivo"]:
            palavra["infinitivo"] = palavra["origem"]
...
# pesquisar a palavra no priberam e recolher toda a informação disponível
def priberam_search(word):
    try:
